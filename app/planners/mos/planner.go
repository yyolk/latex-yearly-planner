...
func (r *Planner) annualSection() (*bytes.Buffer, error) {
	...
	annual, err := sections.NewAnnual(r.year, r.parameters.AnnualParameters)
	if err != nil {
...
func (r *Planner) quarterliesSection() (*bytes.Buffer, error) {
	...
	for _, quarter := range r.year.GetQuarters() {
		...
		header = header.Title(quarterly).Target(quarterly)
...
func (r *Planner) monthliesSection() (*bytes.Buffer, error) {
	...
	for _, month := range r.year.Months() {
		...
		header = header.Title(monthly).Target(monthly)
