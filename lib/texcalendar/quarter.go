...
func (q Quarter) Row() string {
	...
	for _, month := range q.quarter.Months {
		monthsRow = append(monthsRow, tex.AdjustBox(NewMonth(month, common.RightHand).LittleCalendar()))
	}
...
func (q Quarter) Column() string {
	...
	for _, month := range q.quarter.Months {
		months = append(months, NewMonth(month, common.RightHand).LittleCalendar())
	}
